...
An autonomous, multi-agent AI quantitative trading bot for Binance Spot market based on **Heartbeat Architecture (30s)**, **Pod Theory (Sharpe Ratio Filter > 1.0)**, **Rubber Band Mean Reversion (Z-Score Strain)**, and **Fast Capital Recovery Architecture ($80.00 USD Order Allocation, Risk-Reward 2:1, Anti-Overtrading 15m Cooldown, & Safety Circuit Breaker)**.
...
- **🚀 Fast Capital Recovery Strategy ($80.00 USD / Trade)**: Upgraded allocation per trade from $40 to $80 USD (~20% cash allocation) with strict Risk-Reward ratio (2:1).
...
- **🛡️ Institutional Risk Management**:
  ...
  - **Duplicate Buy & Dust Filter**: Cleanly filters out wallet dust (< $15 USD) and restricts maximum active open positions to 2 concurrent coins ($160 USD total allocation).
...
agentic_trading_bot/
├── config/
│   └── settings.py          # Configuration parameters ($80 USD order size, Risk-Reward 2:1)
├── core/
│   └── live_binance.py      # Native Binance REST API client (HMAC SHA256)
├── web/
│   ├── index.html           # Dark financial dashboard UI (v17.0)
│   ├── styles.css           # Modern CSS styling
│   └── app.js               # Frontend application logic & heartbeat timer ($80 USD mode)
...
## 📊 Strategy Parameters (v2.0 - $80 USD Fast Recovery)
...
| **Max Open Positions** | 2 Concurrent Coins | $160 USD total active allocation, maintaining $140+ USDT cash reserve. |
